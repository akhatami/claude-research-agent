...
echo "collision_exit=$?"
```
Expected: no output from comm, then `collision_exit=0` (comm succeeds; the real check is the empty intersection). (POSIX classes used because macOS `sed`/`grep` don't support `\s`.)
