...
## Synthesis documents (auto-saved, never grounding)

A **synthesis** is a durable write-up of an expensive answer, archived to `<active>/synthesis/YYYY-MM-DD-<topic-slug>.md`. Deep answers cost many papers' worth of reading; a synthesis is how that work survives the session.

- **When to write one.** Auto-save, without being asked, whenever answering required **Tier 4** (reading the full text of 2+ papers) or an **Escalation** subagent fan-out. Write it *after* delivering the answer in chat, then tell the user the path in one line. Do not ask permission first — this is a normal part of answering.
- **When NOT to write one.** Answers resolved at Tiers 0–3, single-paper lookups, clarifications, and follow-ups that refine an answer already synthesized. Refining an existing synthesis means **updating that file**, not creating a second one. If a file for today's date and topic already exists, update it in place.
- **A synthesis never grounds.** It is assistant-authored prose, exactly the same tier of trust as a card: it may drift from the papers, and its claims are copies, not sources. Never cite a synthesis, quote it as evidence, or answer a later question from it. When a synthesis bears on a new question, re-derive the claim from `<active>/text/<slug>.md`. You may read one to update or extend it — never to support an assertion.
- **Every synthesis is self-verifying.** It cites `[slug]` and `⟨ghost:key⟩` under the same rules as chat, so every load-bearing claim in it is greppable against `<active>/text/`.
- **Syntheses are corpus-scoped** like everything else: written only into the active corpus, and never read across corpora.

Open each one with this header, so a reader landing on the file cold knows what it is and how far to trust it:

```markdown
# <Title>

*Grounded synthesis over the `<active>` corpus (N papers). Compiled YYYY-MM-DD.
Claims cite `[slug]`; quoted strings are verifiable by grepping `text/<slug>.md`.
Assistant-authored: routes, never grounds — re-verify before reuse.*

**Question addressed:** <the question, as asked>

**One-line answer:** <the answer, hedged exactly as much as the papers warrant>
```
...
- `<active>/INDEX.md` is generated whole from `<active>/index.yaml` by `scripts/generate_views.py` — never hand-edit it. `<active>/LANDSCAPE.md` is a hybrid: the assistant authors the narrative prose, while its `graph` and `ghosts` fenced regions (drawing on `<active>/index.yaml` and `<active>/refs.yaml`) are generated by the same script — never hand-edit those fenced regions. Both are regenerated via the `sync` skill (Phase 6). `<active>/INDEX.md` lists held papers only; ghosts appear only in `<active>/LANDSCAPE.md`. All of the above paths are within the active corpus `corpora/<active>/`; nothing outside it is read or written.
